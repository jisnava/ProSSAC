...
#Do extract details of only first chain(first chain)
import os
import math
import glob
...
for file in os.listdir(directory_path1):
	...
	try:
		...
		#chain_name_old=str(chain_name_old)[0]
		#chain_name_old=str(chain_name_old)
	...
	#newfile2 = open('/home/jisna/Desktop/Akhil_Vinod_2019/DATASET_1442/PDB_CA/' + file + 'chain_name', "a+")
	for j in (lines):
		#print(j)
		#currentline=j.split()
		try:
			...
			if ( (chain_name==chain_name_old) and (res_num >= res_num_old)):
				#L.append(j)
				#print(j)
				#if (str(j[16:17])=='B' or str(j[16:17])=='C'):
					#continue
				#else:
